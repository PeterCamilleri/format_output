...
class Array
  ...
  def puts_format_output_word_wrap(max_width = nil, left_margin = nil)
    puts format_output_word_wrap(max_width, left_margin).join("\n")
  end
  ...
  def format_output_raw_word_wrap(max_width = nil, left_margin = nil)
    result = []

    each do |item|
      result.concat(item.to_s.format_output_raw_word_wrap(max_width, left_margin))
      result << ""
    end
